...
async function setupUserMenu(user) {
  ...
  if (!displayName || !profileImageUrl) {
    console.log("[b-chat.js] User data not cached. Fetching from Firestore...")
    const profile = await profileService.getProfileByUID(user.uid)

    if (profile) {
      const userRole = profile.roles?.includes("business") ? "business" : "customer"
      ...
      if (userRole === "business") {
        ...
        profileImageUrl = businessProfile.profileImageUrl || "/assets/icons/Icon_shop_2.svg"
      } else {
        displayName = profile.displayName
        profileImageUrl = profile.profileImageUrl || "/assets/icons/Icon_shop_2.svg"
      }
